// ==UserScript==
// @name         ChatGPT - Send to Factory
// @namespace    http://tampermonkey.net/
// @version      1.0
// @description  Add "Send to Factory" button to ChatGPT interface
// @author       You
...
(function() {
    ...
    function waitForChatGPT() {
        return new Promise((resolve) => {
            const check = () => {
                ...
                const container = document.querySelector('[data-testid="conversation-turn-2"]') ||
    ...
    function extractConversation() {
        ...
        // Try multiple selectors for ChatGPT's message structure
        const userMessages = document.querySelectorAll('[data-message-author-role="user"]');
        const modelMessages = document.querySelectorAll('[data-message-author-role="assistant"]');
        
        const count = Math.min(userMessages.length, modelMessages.length);
        ...
        // Fallback: try to extract from any visible message bubbles
        if (messages.length === 0) {
            document.querySelectorAll('[data-testid^="conversation-turn-"]').forEach(turn => {
                const roleEl = turn.querySelector('[data-testid^="conversation-turn-"] [data-testid*="avatar"]');
                if (roleEl) {
                    const isUser = roleEl.ariaLabel?.includes('user') || 
                                  roleEl.title?.includes('User');
                    const contentEl = turn.querySelector('.markdown') || turn;
                    messages.push({
    ...
    function createSendButton() {
        ...
        btn.id = 'pf-send-to-factory-chatgpt';
    ...
    function showStatus(message, type) {
        ...
        const existing = document.getElementById('pf-status-chatgpt');
        if (existing) existing.remove();
        
        const status = document.createElement('div');
        status.id = 'pf-status-chatgpt';
    ...
    async function sendToFactory() {
        const btn = document.getElementById('pf-send-to-factory-chatgpt');
        if (!btn) return;
    ...
    async function init() {
        try {
            ...
            console.log('🚀 ChatGPT Send to Factory extension loaded');
        } catch (error) {
            console.error('Failed to initialize ChatGPT extension:', error);
        }
    }
