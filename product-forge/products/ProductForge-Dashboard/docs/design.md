...
### F-2: Project Creation and Model Tier Management
- **FR-21 — Project creation wizard entry.** Provide a guided, multi-step entry point to create a new pipeline project, launched from the control plane (F-1) and resumable across sessions.
- **FR-22 — Idea capture.** Capture the seed idea as free-form text, with optional structured fields (goal, target users, success signals); attach it to the project draft.
- **FR-23 — Project metadata.** Collect and validate the project's name, description, owner, tags, and visibility during creation.
- **FR-24 — Portfolio assignment at creation.** Allow assignment to zero or one portfolio at creation, with inline creation of a new portfolio.
- **FR-25 — Pipeline definition binding.** Let the operator select a pipeline definition and pinned version; default to the platform default definition when none is selected.
- **FR-26 — Model tier selection.** Allow the operator to pick a predefined model tier (e.g., economical/balanced/frontier) as the project's default.
- **FR-27 — Model tier registry.** Maintain the catalog of predefined model tiers with their model/provider composition, cost/quality profile, and availability.
- **FR-28 — Custom model tier creation.** Allow the operator to compose a new tier from the catalog of LLM models and providers, naming and describing it.
- **FR-29 — Model/provider catalog.** Expose the registered LLM models and providers with capabilities, context limits, and regional availability.
- **FR-30 — Tier editing and versioning.** Editing a tier produces a new immutable version; existing projects pinned to an older version are unaffected until explicitly upgraded.
- **FR-31 — Tier validation and compatibility.** Validate that every tier's models/providers exist, are available, and satisfy pipeline stage constraints.
- **FR-32 — Draft persistence and resume.** Persist in-progress creation drafts and allow resuming from the last completed step.
- **FR-33 — Creation validation and readiness.** Validate all required fields and bindings before allowing completion; present an itemized readiness report.
- **FR-34 — Duplicate/clone project.** Create a new project by cloning an existing project's metadata, bindings, and tier (excluding run history).
- **FR-35 — Creation authorization and quota.** Enforce who may create projects, custom tiers, and portfolios, and enforce any configured quotas.
- **FR-36 — Creation idempotency.** Prevent duplicate project/tier creation from retried submissions via idempotency keys.
- **FR-37 — Tier defaults and inheritance.** Apply platform/portfolio default tiers where the operator does not choose, and expose which default applied.
- **FR-38 — Tier change governance on existing projects.** Changing a project's tier requires an explicit, audited action; historical runs retain their original tier.
- **FR-39 — Creation audit trail.** Record who created/modified projects, tiers, and drafts, when, and with what inputs.
- **FR-40 — Creation notifications.** Notify relevant subscribers on project/tier creation, edit, or failure.

### F-3: E2E Pipeline Initiation and Monitoring
- **FR-46 — Run initiation from project.** Start a run for an existing project bound to an immutable pipeline version; accept optional idempotency key, run label, and starting-stage override (only where entry conditions permit). Return a stable run id immediately.
- **FR-47 — Pre-flight readiness validation.** Before any stage executes, validate seed idea, required metadata, resolved model tier, bound pipeline definition/version, and gate policy; block with an itemized readiness report on failure.
- **FR-48 — Run plan materialization.** Expand the pinned pipeline version into a concrete, ordered run plan of stages with resolved parameters.
- **FR-49 — Stage dispatch and execution.** Execute each stage in plan order, honoring entry conditions and dispatching the appropriate stage worker/agent.
- **FR-50 — Live progress and telemetry streaming.** Stream run and stage progress (state, percent, current activity, logs, metrics) to subscribed clients in near real time.
- **FR-51 — HIL gate pause and decision.** Pause at each human-in-the-loop gate, present the decision context, and resume based on the recorded decision.
- **FR-52 — Run completion and outcome.** Finalize a run with a terminal outcome (succeeded/failed/cancelled) and persist the complete stage outcomes.
- **FR-53 — Failure handling and retry.** Detect stage failures, classify them, and support retry per policy or explicit command without corrupting prior records.
- **FR-54 — Run cancellation.** Cancel a run gracefully, stopping in-flight work and marking remaining stages as cancelled.
- **FR-55 — Run pause/resume.** Pause a run at the next safe point and resume from the exact recorded state.
- **FR-56 — Stage retry/skip.** Retry or skip an individual stage where policy permits, recording the action and its effect.
- **FR-57 — Run audit record.** Retain an immutable, ordered audit record of all run/stage events with actors and timestamps.
- **FR-58 — Run state persistence and recovery.** Persist run/stage state so a restart resumes or safely fails-forward without losing or duplicating work.
- **FR-59 — Run idempotency.** Ensure a repeated initiation/command with the same idempotency key does not create duplicate runs or effects.
- **FR-60 — Run concurrency control.** Enforce configured per-project and global concurrency limits, queuing or rejecting excess runs.
- **FR-61 — Stage entry/exit conditions.** Evaluate and enforce each stage's declared entry and exit conditions before and after execution.
- **FR-62 — Run notifications and subscriptions.** Emit run/stage/gate events to in-app, email, and webhook subscribers.
- **FR-63 — Run observability.** Expose run-level metrics, structured logs, and tracing correlated by run/stage id.
- **FR-64 — Run log and artifact retention.** Store run logs and artifacts per retention policy with retrievable references.
- **FR-65 — Run role-based authorization.** Enforce who may initiate, view, and control runs.

### F-4: Portfolio Management
- **FR-66 — Portfolio registry and lifecycle.** Create, read, list, update, archive, restore, and delete portfolios with name, description, owner, and status.
- **FR-67 — Portfolio membership management.** Assign, unassign, and move projects between portfolios, with rules preventing orphaned or ambiguous membership.
- **FR-68 — Portfolio roll-up aggregation.** Compute always-current aggregates across member projects: run counts by status, gate backlog, failure counts, capacity pressure, and last activity.
- **FR-69 — Scoped bulk operations.** Apply archive/pause/resume/run-initiation operations to all or a filtered subset of member projects, with per-item results.
- **FR-70 — Portfolio run and gate views.** Present runs, stages, and pending HIL gates for all member projects in one consolidated view (governed by F-3 data).
- **FR-71 — Portfolio capacity pressure.** Surface concurrency/queue pressure across member projects relative to configured limits.
- **FR-72 — Portfolio failure and last-activity tracking.** Highlight failing projects/runs and the most recent activity per project.
- **FR-73 — Portfolio authorization scope.** Enforce who may view or mutate a portfolio and exercise scoped bulk operations.
- **FR-74 — Portfolio reporting surface.** Provide exportable portfolio reports (state summary, run outcomes, gate backlog) over a chosen time window.
- **FR-75 — Portfolio idempotency/concurrency.** Guarantee idempotent mutations and reject stale updates via revision checks.
- **FR-76 — Portfolio notifications.** Notify on membership changes, bulk-operation completion, and threshold breaches.
- **FR-77 — Portfolio audit trail.** Record all portfolio mutations and scoped commands immutably.
- **FR-78 — Portfolio search/filter.** Query portfolios by name, owner, status, and size.
- **FR-79 — Portfolio health/observability.** Expose metrics for roll-up freshness, aggregation latency, and errors.
- **FR-80 — Portfolio data export.** Export portfolio metadata and roll-up snapshots in a machine-readable format.

### F-5: Multi-Project Runs
- **FR-91 — Multi-project run group creation.** Create a durable parent run group targeting multiple projects, with name, owner, optional portfolio context, scope definition, execution mode, failure policy, shared parameters, and idempotency key.
- **FR-92 — Scope resolution.** Resolve the target project set from explicit ids, portfolio membership, tags, or filters, capturing an immutable scope snapshot.
- **FR-93 — Child run materialization.** Create one child pipeline run per resolved project, applying the run initiation and pre-flight rules of F-3.
- **FR-94 — Shared and overridden parameters.** Apply group-level shared parameters with per-project overrides where permitted.
- **FR-95 — Execution policy.** Enforce group execution mode: parallel fan-out, staged waves, or serialized, within configured concurrency limits.
- **FR-96 — Failure policy.** Apply fail-fast, continue-on-error, or threshold-based abort across child runs per the group's policy.
- **FR-97 — Aggregated monitoring.** Present aggregate and per-child progress, outcomes, and telemetry for the group.
- **FR-98 — Aggregated HIL gates.** Consolidate pending human-in-the-loop gates across child runs into a single group gate view for decision.
- **FR-99 — Parent-child audit trail.** Retain a complete, immutable audit trail linking the parent group to every child run, command, and outcome.
- **FR-100 — Group lifecycle control.** Pause, resume, cancel, and retry the group, propagating to child runs where policy permits.
- **FR-101 — Group idempotency.** Prevent duplicate groups or duplicate child runs from retried submissions.
- **FR-102 — Group concurrency control.** Enforce concurrency limits across all in-flight child runs.
- **FR-103 — Group notifications.** Emit progress and completion notifications aggregated per group and per child.
- **FR-104 — Group authorization.** Enforce who may create, view, and control multi-project run groups.
- **FR-105 — Group observability.** Expose group-level metrics, logs, and tracing correlated by group id.

### F-6: Manual Input and Operation Control
- **FR-116 — Manual input supply.** Allow an operator to supply input to a running or paused run/stage that requests human input.
- **FR-117 — HIL gate decision.** Allow an operator to approve, reject, or conditionally decide a pending human-in-the-loop gate with a rationale.
- **FR-118 — Run/stage pause/resume/cancel.** Issue pause, resume, and cancel commands to a run or to a specified stage.
- **FR-119 — Stage retry/skip.** Retry or skip a specified stage where policy permits, recording the action and effect.
- **FR-120 — Parameter override.** Override run/stage parameters within the bounds permitted by policy, with the change recorded and effect visible.
- **FR-121 — Emergency system-wide halt.** Provide an emergency halt that stops all active runs immediately, with a reason and audit record.
- **FR-122 — Preview/dry-run.** Preview the effect of a command (targets, param diffs, predicted impact) before it is applied.
- **FR-123 — Confirmation.** Require explicit confirmation for destructive or high-impact commands.
- **FR-124 — Command idempotency.** Guarantee that a retried command with the same idempotency key is applied at most once.
- **FR-125 — Command conflict handling.** Detect and resolve conflicts with concurrent commands or stale run state, failing safely.
- **FR-126 — Command audit.** Record every manual command with actor, target, input, preview, and outcome immutably.
- **FR-127 — Role gating.** Restrict command classes to authorized roles and scopes.
- **FR-128 — Bulk manual commands.** Apply a command class to a selected set of runs/stages with per-item results.
- **FR-129 — Command notifications.** Notify stakeholders on command acceptance, completion, and failure.
- **FR-130 — Command authorization scope.** Enforce project/portfolio/run scoping for all command issuance.

### F-7: Auto Mode
- **FR-134 — Auto mode policy definition.** Define a declarative, versioned Auto Mode policy (autonomy level, gate rules, retry/abort rules, guardrails).
- **FR-136 — Autonomy level binding.** Bind an autonomy level (assist / supervised-auto / full-auto) to a project, run, or run group.
- **FR-137 — Automatic HIL gate decisions.** Evaluate HIL gates defined by F-3 against policy and auto-decide where policy permits, escalating when not.
- **FR-138 — Auto-retry/skip/pause/halt/escalate.** Automatically retry, skip, pause, halt, or escalate according to policy triggers.
- **FR-139 — Guardrails.** Enforce budget, time, and risk guardrails, halting or escalating on breach.
- **FR-140 — Bounded automatic parameter overrides.** Apply automatic parameter overrides only within explicitly bounded ranges.
- **FR-141 — Decision ledger.** Maintain an immutable, explainable ledger of every automatic decision with the rule and inputs that produced it.
- **FR-142 — Kill switch / manual takeover.** Provide an immediate kill switch that disables auto mode and hands control to F-6.
- **FR-143 — Fail-closed evaluation.** Escalate rather than guess when policy is ambiguous or unknown.
- **FR-144 — Policy versioning.** Version auto-mode policies; changes do not retroactively alter past decisions.
- **FR-145 — Scope binding.** Bind policies at project, portfolio, or run-group scope with clear precedence.
- **FR-146 — Auto mode notifications.** Notify on escalations, guardrail breaches, and auto-decisions requiring attention.
- **FR-147 — Auto mode audit.** Record policy changes and all auto-mode actions immutably.
- **FR-148 — Auto mode authorization.** Enforce who may define, bind, or disable auto-mode policies.
- **FR-149 — Auto mode observability.** Expose metrics for auto-decisions, escalations, and guardrail events.

### F-8: Dashboard UX Component System
- **FR-159 — Design token foundation.** Provide a versioned design-token set (color, spacing, typography, elevation, motion) consumed by all components.
- **FR-161 — Primitive components.** Deliver the primitive library (buttons, inputs, tags, icons, tooltips, etc.).
- **FR-162 — Composed components.** Deliver composed patterns (forms, tables, dialogs, tabs, breadcrumbs, banners).
- **FR-163 — Feature components.** Deliver feature components (run timeline, stage card, gate panel, portfolio roll-up, telemetry chart).
- **FR-164 — Loading states.** Provide skeletons and progress affordances for content and long operations.
- **FR-165 — Empty states.** Provide empty-state patterns with guidance and CTAs for every list/panel.
- **FR-166 — Error states.** Provide inline, toast, and page-level error presentations with recovery actions.
- **FR-167 — Real-time update behaviours.** Standardize how live telemetry updates render without disruptive layout shifts.
- **FR-168 — Theming.** Support light/dark (and additional) themes via tokens without component forks.
- **FR-169 — Responsiveness.** Provide responsive layouts from mobile to desktop breakpoints.
- **FR-170 — Accessibility.** Meet WCAG 2.1 AA: contrast, keyboard operability, focus visibility, ARIA, and screen-reader semantics.
- **FR-171 — Internationalization.** Support localized strings, RTL, and locale-aware number/date/currency formatting.
- **FR-172 — Motion.** Provide consistent, reduced-motion-respecting animation guidance.
- **FR-173 — Pipeline-state presentation contract.** Define how project/run/stage states are color-coded, labelled, and announced consistently.
- **FR-160 — Telemetry presentation contract.** Define how run/telemetry data is visualized consistently across surfaces.
- **FR-174 — Command presentation contract.** Define consistent affordances, confirmation, and result feedback for operator commands (semantics owned by F-6).
- **FR-175 — Component versioning.** Version components and tokens with documented migration paths.
- **FR-176 — Component documentation.** Provide usage docs, examples, and accessibility notes for every component.

### F-9: Global AI Chat Companion
- **FR-184 — Availability.** Provide an always-reachable chat companion openable from any dashboard surface.
- **FR-185 — Context binding.** Automatically bind the conversation to the operator's current context (project, run, stage, gate, portfolio, run group, auto-mode session).
- **FR-186 — Grounded Q&A.** Answer questions about authoritative pipeline state by reading published surfaces of other features.
- **FR-187 — Citations.** Cite the authoritative object behind every factual statement about pipeline state.
- **FR-188 — Explanation and diagnosis.** Explain behaviour and diagnose failures using authoritative state.
- **FR-189 — Command drafting.** Draft commands for the operator without executing them.
- **FR-190 — Hand-off to command surface.** Pass drafted commands to the F-6 manual command surface for preview and confirmation.
- **FR-191 — Refusal to invent.** Refuse to invent state, commands, or capabilities; state clearly when information is unavailable.
- **FR-192 — Authorization scope enforcement.** Never expose data or draft commands beyond the operator's authorization scope.
- **FR-193 — Graceful degradation.** Degrade to deterministic help and search when the AI backend is unavailable.
- **FR-194 — Help and search fallback.** Provide navigable help topics and search when AI is unavailable.
- **FR-195 — Conversation history.** Retain scoped, retrievable conversation history honoring privacy controls.
- **FR-196 — Feedback.** Allow operators to rate answers and flag incorrect grounding.
- **FR-197 — Internationalization.** Respond in the operator's configured language.
- **FR-198 — Accessibility.** Be fully keyboard- and screen-reader-operable.
- **FR-199 — Observability.** Expose metrics for grounding accuracy, refusal rate, latency, and errors.
- **FR-200 — Privacy controls.** Provide controls over conversation retention and data sent to the AI backend.

### F-10: Voice Companion
- **FR-209 — Activation and microphone lifecycle.** Provide push-to-talk and/or wake-word activation with an explicit, revocable microphone lifecycle.
- **FR-210 — Streaming speech-to-text.** Transcribe speech to text in a streaming manner with interim and final results.
- **FR-211 — Spoken synthesis.** Speak grounded answers synthesized from the F-9 response.
- **FR-212 — Spoken citations.** Verbally attribute statements to their authoritative objects.
- **FR-214 — Barge-in.** Allow the operator to interrupt playback and speak; the system stops and listens.
- **FR-215 — Hands-free HIL gate announcements.** Announce pending HIL gates aloud and accept a spoken decision, routed through F-6 for confirmation/audit.
- **FR-216 — Voice-driven navigation.** Navigate dashboard surfaces by voice.
- **FR-217 — Voice drafting of commands.** Capture a spoken command and hand it to F-6 as a draft for preview/confirmation.
- **FR-213 — Multilingual speech.** Support multiple speech locales for recognition and synthesis.
- **FR-218 — Transcript/audio privacy controls.** Provide controls over transcript/audio retention and sharing.
- **FR-219 — Graceful degradation to text.** Fall back to the text companion when voice services are unavailable.
- **FR-220 — Voice permission management.** Request, display, and honor microphone permissions; never record without consent.
- **FR-221 — Session control.** Provide explicit start/stop and a visible listening indicator.
- **FR-222 — Voice observability.** Expose metrics for recognition latency, synthesis latency, and failures.
- **FR-223 — Voice accessibility.** Ensure voice features do not gate access and always have operable text alternatives.

### F-11: Mobile Dashboard Companion
- **FR-224 — Responsive read views.** Render the authoritative pipeline state in a mobile-first, read-optimized layout.
- **FR-225 — Project/portfolio browsing.** Browse projects and portfolios with search, filter, and detail views.
- **FR-226 — Run/stage views.** View run and stage state, telemetry, and logs (summarized for mobile).
- **FR-227 — HIL gate views.** View pending HIL gates and their context.
- **FR-228 — Failure views.** View failures with diagnosis and links to detail.
- **FR-229 — Auto-mode decision views.** View auto-mode decisions and escalations from the F-7 ledger.
- **FR-230 — Multi-project run group views.** View run-group aggregate and per-child state.
- **FR-231 — Notification-driven alerts.** Push notifications for gates, failures, escalations, and completions.
- **FR-232 — Command delegation to F-6.** Route every command action to the F-6 surface for preview, confirmation, and audit; never execute directly.
- **FR-233 — Offline read-only last-known state.** Show the last-known state read-only when offline.
- **FR-234 — Connectivity degradation.** Indicate connectivity/offline status and recover cleanly on reconnect.
- **FR-235 — Mobile security.** Support device lock, session timeout, and secure token storage.
- **FR-236 — Mobile privacy.** Prevent sensitive data leakage via notifications and screenshots where configured.
- **FR-237 — Mobile accessibility.** Meet touch-target, contrast, and screen-reader requirements.
- **FR-238 — Notification preferences.** Let operators configure notification categories and quiet hours.
...
### F-12: Test Management
- **FR-239 — Test case registry.** Register, read, list, update, and retire test cases with owner, tags, and severity.
- **FR-240 — Versioned test definitions.** Version test definitions immutably; existing bindings pin to a version.
- **FR-241 — Suite composition.** Compose test cases into suites.
- **FR-242 — Test plan composition.** Compose suites/cases into test plans with sequencing and selection rules.
- **FR-243 — Plan binding.** Bind plans to pipeline stages and projects so a stage can consume a governed quality gate.
- **FR-244 — Test execution.** Execute tests on demand, on schedule, or as a governed stage of a pipeline run (driven by F-3).
- **FR-245 — Result capture and evidence.** Capture deterministic outcomes and attach evidence (logs, artifacts, diffs).
- **FR-246 — Quality gate evaluation.** Evaluate plan-level quality gates and publish pass/fail verdicts that stage advancement consumes.
- **FR-247 — Coverage tracking.** Track coverage over pipeline artifacts and report gaps.
- **FR-248 — Regression and flakiness management.** Detect regressions and flaky tests and manage them (quarantine, re-run).
- **FR-249 — Waiver/approval.** Allow authorized waivers of failing gates with rationale and expiry.
- **FR-250 — Reporting.** Provide test reports (pass rate, trends, coverage, gate history).
- **FR-251 — Audit trail.** Retain an immutable audit trail of test definitions, runs, results, and gate verdicts.
- **FR-252 — Definition API.** Publish a stable API of test case/suite/plan definitions.
- **FR-253 — Run API.** Publish a stable API to start and query test runs.
- **FR-254 — Result/gate verdict API.** Publish a stable API of results and gate verdicts for consumers.
- **FR-255 — Scheduling.** Schedule recurring test plans with run-window and concurrency rules.
- **FR-256 — Test authorization.** Enforce who may define, run, waive, and view tests.

### F-13: API-First Access
- **FR-264 — Versioning and lifecycle.** Version the API surface and manage introduce/deprecate/retire lifecycle.
- **FR-265 — Contract publication.** Publish machine-readable, contract-verified API specifications.
- **FR-266 — Client authentication.** Authenticate API clients via supported credential types.
- **FR-267 — Scope-based authorization.** Enforce per-client scopes mirroring dashboard capabilities.
- **FR-268 — Credential lifecycle.** Create, rotate, revoke, and expire client credentials.
- **FR-269 — Uniform envelope.** Apply a consistent request/response envelope across all operations.
- **FR-270 — Error model.** Provide a uniform, machine-readable error model with stable codes.
- **FR-271 — Pagination/filtering/fieldsets.** Support pagination, filtering, sorting, and sparse fieldsets uniformly.
- **FR-272 — Idempotency.** Support idempotency keys on mutating operations.
- **FR-273 — Concurrency control.** Support optimistic concurrency via ETags/revisions.
- **FR-274 — Async long-running operations.** Model long-running work as pollable operations with status and results.
- **FR-275 — Streaming telemetry.** Provide a streaming surface for live run telemetry.
- **FR-276 — Webhook/event subscriptions.** Allow clients to subscribe to events via webhooks with retries and signing.
- **FR-277 — Bulk operations.** Provide bulk endpoints with per-item results.
- **FR-278 — Dry-run preview.** Provide dry-run/preview for supported mutating operations.
- **FR-279 — Rate limiting and quotas.** Enforce per-client rate limits and quotas with clear signalling.
- **FR-280 — Capability discovery.** Expose a discovery endpoint describing capabilities and versions.
- **FR-281 — Activity auditing.** Audit all API activity (actor, operation, outcome).
- **FR-282 — SDK/CLI generation.** Generate SDKs and a CLI from the published contract.
- **FR-283 — Sandbox environment.** Provide an isolated sandbox for safe client development.

### F-14: API Reuse and Extension Analysis
- **FR-289 — Contract inventory snapshot.** Continuously snapshot the versioned API contracts published by F-13.
- **FR-290 — Demand registry.** Maintain a registry of registered consumer demand for capabilities.
- **FR-291 — Matching and gap detection.** Match demand against contracts and detect gaps.
- **FR-292 — Reuse/extension classification.** Classify each proposed capability as reuse, additive extension, versioned change, or new.
- **FR-293 — Duplication/overlap findings.** Detect duplicate or overlapping operations across contracts.
- **FR-294 — Coverage scoring.** Compute per-area coverage scores.
- **FR-295 — Review/decision record.** Record review outcomes and decisions for each proposed capability.
- **FR-296 — Published reuse guidance.** Publish guidance recommending reuse before new introduction.
- **FR-297 — Read-only guarantee.** Never mutate, deprecate, or publish contracts on another feature's behalf.
- **FR-298 — Refresh/scheduling.** Refresh analysis on contract changes or a schedule.
- **FR-299 — Reporting.** Provide analysis reports (gaps, overlaps, coverage).
- **FR-300 — Authorization.** Enforce who may view or contribute demand records.
- **FR-301 — Audit.** Audit analysis runs and decisions.
- **FR-302 — Analysis API.** Publish a stable API for snapshots, findings, and guidance.

### F-15: Deployment Options
- **FR-304 — Deployment-target catalog.** Define supported deployment targets (e.g., managed cloud, self-hosted, air-gapped) and their guarantees.
- **FR-305 — Topology/sizing profiles.** Provide selectable topology and sizing profiles.
- **FR-306 — Provisioning/bootstrap.** Provide a provisioning/bootstrap path per target.
- **FR-307 — Config and secret management.** Define a configuration and secret-management contract per target.
- **FR-308 — Upgrade/rollback.** Support versioned upgrade with safe rollback.
- **FR-309 — Scaling.** Support horizontal/vertical scaling per profile.
- **FR-310 — High availability.** Provide HA configurations with defined failure domains.
- **FR-311 — Backup/restore.** Provide backup and point-in-time restore.
- **FR-312 — Observability wiring.** Wire metrics, logs, and tracing per target.
- **FR-313 — Network/TLS and identity wiring.** Configure network, TLS, and identity integration per target.
- **FR-314 — Data residency.** Enforce data-residency placement options.
- **FR-315 — Air-gapped/offline delivery.** Support fully offline delivery and updates.
- **FR-316 — Entitlement binding.** Bind entitlements/licenses to installations.
- **FR-317 — Drift detection.** Detect drift between declared and running configuration.
- **FR-318 — Multi-environment promotion.** Support promotion across environments.
- **FR-319 — Hardening profiles.** Provide hardening profiles (CIS-style baselines).
- **FR-320 — Cost estimation.** Estimate cost per profile.
- **FR-321 — Decommissioning.** Support safe decommissioning and data disposal.
- **FR-322 — Deployment manifest/API.** Publish a machine-readable manifest describing a running installation and a stable deployment API.
- **FR-323 — Health/parity reports.** Publish health and cross-target parity reports.

### F-16: Voice Reference Alignment
- **FR-329 — Reference voice registry.** Register and manage reference voices and their locale.
- **FR-330 — Sample ingestion.** Ingest source audio samples for a reference.
- **FR-331 — Preprocessing/normalization.** Preprocess and normalize samples (level, denoise, format).
- **FR-332 — Phonetic/transcript alignment.** Align phonetics and transcripts to the samples.
- **FR-333 — Speaker-embedding extraction.** Extract speaker embeddings per sample.
- **FR-334 — Alignment scoring and verdicts.** Compute objective alignment scores and pass/review/fail verdicts.
- **FR-335 — Immutable profile versioning.** Compose immutable reference-profile versions.
- **FR-336 — Approval/retirement workflow.** Approve and retire profiles with role gating.
- **FR-337 — Binding to languages/locales.** Bind profiles to languages/locales and the Voice Companion's resolution rules.
- **FR-338 — Drift monitoring.** Monitor profile drift against fresh samples.
- **FR-339 — Cross-sample consistency.** Enforce cross-sample consistency checks.
- **FR-340 — Search and import/export.** Search references and import/export profiles.
- **FR-341 — Role-gated access.** Restrict alignment operations to authorized roles.
- **FR-342 — Immutable audit trail.** Retain an immutable audit trail of alignments and decisions.
- **FR-343 — Alignment settings/presets.** Provide alignment settings and reusable presets.
- **FR-344 — Batch alignment.** Run alignment in batches with per-item results.
- **FR-345 — Failure diagnosis.** Diagnose alignment failures with actionable causes.
- **FR-346 — Preview synthesis.** Preview-synthesize from a candidate profile.
- **FR-347 — Completion notifications.** Notify on alignment completion and verdicts.
- **FR-348 — Reference API.** Publish a stable API of reference profiles, alignments, and verdicts for F-10 to consume.
...
- **NFR-38 (Performance):** Portfolio roll-up read p95 < 800 ms for a portfolio of 100 member projects.
- **NFR-39 (Performance):** Background roll-up cache refresh completes within ≤ 30 s of a member-state change; explicit operator-triggered refresh returns p95 < 2 s.
- **NFR-40 (Scalability):** Supports ≥ 1,000 portfolios and ≥ 5,000 projects per portfolio without breaching NFR-38.
- **NFR-41 (Availability):** When the roll-up pipeline is degraded, the portfolio view serves last-known-good data with an explicit staleness indicator instead of failing or showing misleading zeros.
- **NFR-42 (Consistency):** Roll-ups are eventually consistent with a bounded lag ≤ 30 s and are always displayed with an "as of" timestamp.
- **NFR-43 (Security):** Portfolio membership changes and scoped bulk operations are restricted to portfolio owner/admin roles and enforced per affected project.
- **NFR-44 (Security):** Roll-ups apply row-level filtering so an aggregate never reveals the existence or state of a project the viewer is not authorized to read.
- **NFR-45 (Auditability):** Membership changes, bulk operations and roll-up-affecting configuration changes produce immutable audit records.
- **NFR-46 (Data residency):** Portfolio metadata and derived roll-ups are stored in the residency region; no aggregation crosses region boundaries.
- **NFR-47 (Deployment/environment):** Portfolio management works in cloud, self-hosted and air-gapped profiles with equivalent semantics.
- **NFR-48 (Safety):** Bulk operations are bounded (maximum target count per operation), previewable before execution, and cancellable while in progress.
- **NFR-49 (Reporting):** Roll-up export for 5,000 projects completes ≤ 5 s and is available in documented CSV and JSON formats.

### F-5: Multi-Project Runs

- **NFR-53 (Performance):** Multi-project run group creation p95 < 1 s; child run fan-out begins within ≤ 5 s of group start.
- **NFR-54 (Scalability):** Supports ≥ 500 child runs per group and ≥ 20 concurrently executing groups per installation.
- **NFR-55 (Availability):** A group tolerates partial child failure and continues according to its declared failure policy; group state remains queryable throughout.
- **NFR-56 (Consistency):** Parent group status is derived deterministically from child states by a documented rule; no phantom or undefined aggregate states are observable.
- **NFR-57 (Idempotency):** Group creation and child fan-out are idempotent; restart or replay never produces duplicate child runs.
- **NFR-58 (Resource fairness):** Group dispatch respects global concurrency limits and cannot starve single-project runs; a documented fairness policy is enforced.
- **NFR-59 (Security):** A group's project scope is authorized at creation and re-validated at dispatch; an actor cannot include projects they cannot command.
- **NFR-60 (Auditability):** Parent-child lineage, shared/overridden parameters and per-child outcomes produce an immutable audit trail.
- **NFR-61 (Data residency):** Group metadata, aggregation and child telemetry are stored in the residency region.
- **NFR-62 (Deployment/environment):** Multi-project runs operate identically in air-gapped deployments with no external coordination service required.
- **NFR-63 (Observability):** Aggregated group progress, per-child drill-down, fan-out latency and reconciliation events are exposed as metrics and traces.
- **NFR-64 (Recovery):** After a crash, group reconciliation converges to a consistent parent/child state within ≤ 2 minutes without duplicating child work.

### F-6: Manual Input and Operation Control

- **NFR-68 (Performance):** Manual command acknowledgement p95 < 300 ms; dry-run/preview p95 < 1 s.
- **NFR-69 (Scalability):** ≥ 100 concurrent operators can issue commands without visible contention or lost commands.
- **NFR-70 (Availability):** The command surface is available whenever the control plane is; when execution is unavailable, commands are queued with an accurate pending state rather than rejected silently.
- **NFR-71 (Concurrency/conflict):** Concurrent conflicting commands are resolved by optimistic version check and return a deterministic 409-style conflict with the current state, never a silent last-write-wins.
- **NFR-72 (Security):** Every action is role-gated; destructive and system-wide actions require an elevated role plus explicit confirmation.
- **NFR-73 (Security):** Emergency system-wide halt is restricted to authorized roles with an optionally configurable two-person rule.
- **NFR-74 (Auditability):** Every manual command (including dry-runs and rejected commands) is immutably audited with actor, target, parameters, before/after state and result.
- **NFR-75 (Safety):** Preview/dry-run never mutates authoritative state or emits externally observable side effects.
- **NFR-76 (Data residency):** Command audit records, including operator input payloads, are stored in the residency region with no cross-border egress.
- **NFR-77 (Deployment/environment):** The manual command surface is available in all deployment profiles, including air-gapped, with no external dependency.
- **NFR-78 (Reliability):** Commands are atomic — a failed command leaves the target run/stage in its prior well-defined state and never in a partial state.
- **NFR-79 (Retention):** Command audit records are retained ≥ 400 days and are not alterable by any later command.

### F-7: Auto Mode

- **NFR-83 (Performance):** Policy evaluation per autonomous decision p95 < 200 ms; automated HIL gate resolution completes within ≤ 5 s of gate availability.
- **NFR-85 (Scalability):** ≥ 10,000 automated decisions per day and ≥ 200 concurrently auto-managed runs per installation.
- **NFR-86 (Availability/safety):** Auto Mode fails closed: loss or unavailability of the policy evaluator escalates to a human rather than auto-approving or auto-advancing.
- **NFR-87 (Determinism):** Identical inputs plus the same recorded policy version yield the same decision; the policy version used is recorded on every decision.
- **NFR-88 (Security):** Changing autonomy level or policy requires an elevated role and is audited; the kill switch is always reachable by authorized operators and reverts to manual control immediately.
- **NFR-89 (Safety):** Budget, time and risk guardrails are enforced by the platform and cannot be widened by a policy; a policy attempting to exceed them is rejected on save.
- **NFR-90 (Auditability/explainability):** Every decision is recorded in an immutable ledger citing the exact rule, inputs and outcome, sufficient for a human to reconstruct the reasoning.
- **NFR-91 (Data residency):** The decision ledger and policy definitions are stored in the residency region.
- **NFR-92 (Deployment/environment):** Auto Mode evaluates policies locally without external services and operates in air-gapped deployments.
- **NFR-93 (Observability):** Decision volume, escalation rate, guardrail trips and kill-switch activations are emitted as metrics with alerting.
- **NFR-94 (Recovery):** Auto Mode resumes decision-making after a restart without duplicating previously applied actions (idempotent decisions).

### F-8: Dashboard UX Component System

- **NFR-99 (Performance UX):** First Contentful Paint < 1 s, Largest Contentful Paint < 2.5 s, Time to Interactive < 3 s on a representative 3G-class connection; page load < 2 s on broadband.
- **NFR-105 (Performance UX):** Interaction (input/click) response < 100 ms; Cumulative Layout Shift < 0.1 across all standard surfaces.
- **NFR-101 (Scalability):** Tables/lists render 10,000 rows via virtualization without frame drops; design-token or theme changes apply without full page reload.
- **NFR-106 (Accessibility):** WCAG 2.1 AA across all composed surfaces — text contrast ≥ 4.5:1, UI contrast ≥ 3:1, complete keyboard navigation, semantic HTML with ARIA where needed, visible 2 px focus indicators, skip-to-content link, no flashing > 3 Hz.
- **NFR-98 (Availability):** The application shell is cacheable and degrades to read-only with a persistent offline banner rather than an error page.
- **NFR-100 (Compatibility):** Supported on the latest two versions of Chrome, Edge, Firefox and Safari; responsive from 320 px to 4K.
- **NFR-107 (Security):** No secrets in client bundles; strict Content Security Policy; all domain-sourced content is output-encoded to prevent XSS.
- **NFR-108 (Internationalization):** Locale selector in settings; dates, times, numbers and currency formatted per user locale; layout is RTL-ready.
- **NFR-109 (Theming):** Light, dark and high-contrast themes, persisted per user and applied without flash-of-wrong-theme.
- **NFR-103 (Real-time UX):** Streaming updates do not cause layout thrash; optimistic UI reconciles to authoritative state and visibly reverts on rejection.
- **NFR-102 (Deployment/environment):** All assets and fonts are served from the installation's own origin; no third-party CDN is required, so air-gapped deployments render fully.

### F-9: Global AI Chat Companion

- **NFR-113 (Performance):** First response token p95 < 2 s; complete grounded answer p95 < 6 s for typical context sizes.
- **NFR-114 (Scalability):** ≥ 200 concurrent chat sessions per installation with context scoped to the objects relevant to the operator's current view.
- **NFR-115 (Availability):** When the AI backend is unavailable, the companion degrades to deterministic help and search with a clear indicator, never presenting ungrounded output as authoritative.
- **NFR-116 (Accuracy/grounding):** Every factual claim about pipeline state cites the authoritative object it came from; if state cannot be resolved the companion refuses rather than inventing state or commands.
- **NFR-117 (Security):** The companion operates strictly within the operator's authorization scope and resists prompt-injection attempts that would escalate to unauthorized data access or command execution.
- **NFR-118 (Safety):** The companion drafts commands only; execution requires preview and confirmation through the manual command surface (F-6) and is never self-executed.
- **NFR-119 (Privacy):** Conversation content is handled per the installation's documented policy; tenant data is not used for model training without explicit opt-in.
- **NFR-120 (Data residency):** Prompts, retrieved context and transcripts are processed and stored within the configured residency region; model routing respects residency constraints.
- **NFR-121 (Auditability):** Command drafts, cited objects and refusal events are logged with the requesting operator and session.
- **NFR-122 (Deployment/environment):** Supports a locally hosted or self-hosted model endpoint so the companion functions in fully air-gapped deployments.

### F-10: Voice Companion

- **NFR-128 (Performance):** Partial speech-to-text transcript latency < 500 ms; end-of-speech to start of spoken answer p95 < 3 s.
- **NFR-129 (Performance):** Barge-in (operator interrupting spoken output) stops playback within < 300 ms.
- **NFR-130 (Scalability):** ≥ 50 concurrent voice sessions per installation.
- **NFR-131 (Availability):** When voice services are unavailable, the companion transparently degrades to the text experience of F-9 with a clear indicator.
- **NFR-132 (Accessibility):** All voice-driven functionality has an equivalent keyboard/text path; spoken output is accompanied by captions/transcript.
- **NFR-133 (Security):** Microphone access requires explicit per-session consent, shows a persistent capture indicator, and never records in the background.
- **NFR-134 (Privacy):** Audio retention is off by default; when enabled it is bounded and documented, transcripts follow policy, and redaction controls are available.
- **NFR-135 (Safety):** Voice-originated commands are drafted only and never executed by voice alone; destructive actions require visual, non-voice confirmation.
- **NFR-136 (Internationalization):** Speech recognition and synthesis support the documented locales, with locale-appropriate pronunciation and number/date rendering.
- **NFR-137 (Data residency):** Audio and transcripts are processed in the residency region; any speech provider used must be region-pinned and documented.
- **NFR-138 (Deployment/environment):** Supports on-device or locally hosted STT/TTS so voice functions in air-gapped and offline conditions where configured.
- **NFR-139 (Auditability):** Voice-originated command drafts, hands-free HIL announcements and accept/reject outcomes are logged with operator and session identity.

### F-11: Mobile Dashboard Companion

- **NFR-143 (Performance):** Cold start < 3 s on a mid-tier device; list scrolling maintains 60 fps on supported hardware.
- **NFR-144 (Performance):** Push notification delivery p95 < 10 s from the originating pipeline event.
- **NFR-145 (Scalability):** Browsing ≥ 10,000 objects via paging without memory growth; offline cache bounded (target ≤ 50 MB) with documented eviction.
- **NFR-146 (Availability):** Read-only access to last-known state is available offline, with a clear offline indicator and graceful reconnect/resync.
- **NFR-147 (Security):** Device-bound sessions, biometric unlock, remote wipe, encrypted local storage, TLS pinning where supported; no domain data in unencrypted storage or logs.
- **NFR-148 (Security):** Sensitive actions require re-authentication; the app executes no commands itself and delegates every command to F-6 for preview, confirmation and audit.
...
- **NFR-149 (Performance):** Test definition reads p95 < 300 ms; result ingestion p95 < 500 ms; plan-level gate verdict computed within ≤ 5 s of the last required result.
- **NFR-150 (Scalability):** ≥ 100,000 test cases, ≥ 1,000,000 retained results, and ≥ 500 concurrent test executions per installation.
- **NFR-151 (Availability/safety):** When the test subsystem is degraded, a governed quality gate fails closed with an explicit verdict and never reports an implicit pass.
- **NFR-152 (Determinism):** A given artifact plus environment plus settings reproduces the same outcome; flakiness is quantified and surfaced rather than silently retried to green.
- **NFR-153 (Security):** Test code and artifacts execute in a sandboxed, least-privilege environment; secrets are injected at runtime and never embedded in definitions or results; pipeline credentials are never exposed to tests.
- **NFR-154 (Auditability):** Definitions, executions, results, evidence, waivers and approvals are recorded immutably with actor and timestamp.
- **NFR-155 (Data residency):** Test results and evidence artifacts are stored in the configured residency region.
- **NFR-156 (Deployment/environment):** Execution agents run air-gapped and can be hosted on-premises with no external runner dependency.
- **NFR-157 (Retention):** Result and evidence retention is configurable, defaults to 400 days, and applies secure destruction on expiry.
- **NFR-158 (Integrity):** Quality-gate verdicts are hash-chained or signed; a past verdict cannot be retroactively altered without producing a new, separately audited record.

### F-13: API-First Access

- **NFR-164 (Performance):** Read endpoints p95 < 300 ms and write endpoints p95 < 500 ms at rated load.
- **NFR-165 (Scalability):** Sustains ≥ 200 req/s with burst capacity to ≥ 1,000 req/s per installation; list endpoints require pagination and reject unbounded result sets.
- **NFR-166 (Availability):** API availability ≥ 99.9% monthly; published versions remain available for their documented deprecation window.
- **NFR-167 (Security):** Client authentication via OIDC/OAuth2 with scope-based authorization, short-lived access tokens, rotating refresh credentials, and immediate revocation on credential compromise.
- **NFR-168 (Security):** Per-client rate limiting and quotas are enforced; a uniform error envelope is returned that never leaks internal implementation details or stack traces.
- **NFR-169 (Reliability):** All writes accept idempotency keys; optimistic concurrency via ETag/If-Match; long-running operations follow an asynchronous job pattern with pollable status.
- **NFR-170 (Contract integrity):** A machine-readable contract is published and verified in CI against live behaviour; an undocumented or unverified endpoint is treated as a defect; breaking changes require a new version with ≥ 2 minor releases of deprecation notice.
- **NFR-171 (Auditability):** API activity is audited per client credential with actor, scope, target and outcome; audit records are immutable.
- **NFR-172 (Data residency):** Requests and resulting data are processed and stored in the client's configured residency region; no third-party egress without explicit opt-in.
- **NFR-173 (Deployment/environment):** The API is available with equivalent contracts in cloud, self-hosted and air-gapped profiles and has no mandatory external dependency.
- **NFR-174 (Streaming):** Telemetry streams support reconnect and resume-from-cursor so a dropped connection does not lose events or duplicate them into the consumer's state.
- **NFR-175 (Webhooks/events):** Delivery is at-least-once with signed payloads, exponential backoff retry, a dead-letter path, and operator-triggered replay.

### F-14: API Reuse and Extension Analysis

- **NFR-179 (Performance):** Inventory snapshot build ≤ 60 s; gap/reuse query p95 < 2 s.
- **NFR-180 (Scalability):** Analyzes ≥ 5,000 API operations against ≥ 10,000 demand entries without breaching NFR-179.
- **NFR-181 (Availability):** The analysis layer is read-only and cannot block API publication; its unavailability never degrades F-13.
- **NFR-182 (Accuracy):** Every reuse/extension classification references the exact contract version and evidence; no finding is emitted without a resolvable citation.
- **NFR-183 (Security):** The layer holds read-only scope over contracts and has no ability to mutate, deprecate or publish an API on another feature's behalf.
- **NFR-184 (Auditability):** Review decisions and published reuse guidance carry an immutable record of inputs, reviewer and outcome.
- **NFR-185 (Freshness):** Inventory snapshots refresh within ≤ 15 minutes of a contract change, or on explicit operator demand.
- **NFR-186 (Data residency):** Inventory snapshots and demand registry data are stored in the configured residency region.
- **NFR-187 (Deployment/environment):** Operates in all deployment profiles, including air-gapped, using only locally available contract sources.

### F-15: Deployment Options

- **NFR-190 (Performance):** Provisioning of a standard profile completes within ≤ 30 minutes; upgrade-plan generation < 60 s.
- **NFR-191 (Scalability):** Documented sizing profiles span single-node through multi-region HA, each with published capacity limits and supported component counts.
- **NFR-192 (Availability):** The HA profile delivers ≥ 99.95% availability with tested failover; each profile publishes explicit RPO/RTO guarantees.
- **NFR-193 (Recovery):** Backup/restore is automated and drill-verified; standard profile RPO ≤ 15 minutes and RTO ≤ 4 hours.
- **NFR-194 (Security):** Secrets are sourced from an external secret manager or KMS with no plaintext at rest; hardened profiles are CIS-aligned; container images are signed and accompanied by an SBOM.
- **NFR-195 (Security):** TLS is enforced on all external interfaces and mTLS where supported internally; identity federation is supported for operator authentication.
- **NFR-196 (Data residency):** A deployment is pinned to the operator's chosen region; no data movement across regions occurs without explicit configuration and documented consent.
- **NFR-197 (Deployment/environment):** Cloud, self-hosted and air-gapped/offline delivery are all supported, with air-gapped installs delivered as signed bundles containing every dependency.
- **NFR-198 (Configuration integrity):** Continuous drift detection produces a parity report; promotion between environments is gated by approval and reproducible from a recorded configuration revision.
- **NFR-199 (Observability):** Metrics, logs, traces and health/parity endpoints are wired by default for every profile, requiring no additional instrumentation.
- **NFR-200 (Upgrade/rollback):** Upgrades are reversible; rollback completes within ≤ 30 minutes with no data loss against the stated RPO.
- **NFR-201 (Cost transparency):** A cost estimate is available for each profile before provisioning, based on documented sizing assumptions.
- **NFR-202 (Decommissioning):** Decommissioning performs secure data destruction and issues a verifiable destruction certificate.
- **NFR-203 (Entitlement):** Entitlement binding is enforced at runtime, with offline validation supported for air-gapped installations.

### F-16: Voice Reference Alignment

- **NFR-205 (Performance):** Alignment of a 5-minute source sample completes within ≤ 3 minutes; speaker-embedding extraction ≤ 30 s per sample.
- **NFR-206 (Scalability):** Supports ≥ 10,000 reference profiles and ≥ 100 concurrent alignment jobs per installation without breaching NFR-205.
- **NFR-207 (Availability):** Alignment jobs are queued and retried; a failure is isolated to the affected reference and never blocks other profiles, approvals or the Voice Companion's existing bindings.
- **NFR-208 (Quality determinism):** The same samples plus the same recorded alignment-settings version produce the same alignment score; settings versions are recorded on every verdict.
- **NFR-209 (Security):** Access to reference profiles and source audio is role-gated; source audio cannot be downloaded or exported without an authorized, audited action.
- **NFR-210 (Privacy/consent):** A reference cannot be created without a recorded consent/licence basis; retention and deletion are enforced, and deletion removes derived embeddings as well as source audio.
- **NFR-211 (Data residency):** Source audio, embeddings and derived profiles are stored in the configured residency region with no egress to external services.
- **NFR-212 (Auditability):** Ingestion, preprocessing, alignment, scoring, approval, binding and retirement all produce immutable audit records.
- **NFR-213 (Deployment/environment):** Batch alignment runs air-gapped with no external service dependency; all models and tooling ship with the installation.
- **NFR-214 (Storage integrity):** Reference artifacts are hash/signature verified on ingest and read; reference-profile versions are immutable once approved.
- **NFR-215 (Retention):** Retired references are retained per policy and then securely destroyed, with the destruction recorded in the audit trail.

### Category Coverage Map

| Quality category | Representative NFRs |
|---|---|
| Performance | NFR-1, NFR-2, NFR-16, NFR-17, NFR-31, NFR-32, NFR-38, NFR-39, NFR-53, NFR-68, NFR-83, NFR-99, NFR-105, NFR-113, NFR-128, NFR-129, NFR-143, NFR-144, NFR-149, NFR-164, NFR-179, NFR-190, NFR-205 |
| Scalability / availability | NFR-3, NFR-4, NFR-18, NFR-19, NFR-33, NFR-34, NFR-35, NFR-40, NFR-41, NFR-54, NFR-55, NFR-69, NFR-70, NFR-85, NFR-86, NFR-101, NFR-98, NFR-114, NFR-115, NFR-130, NFR-131, NFR-145, NFR-146, NFR-150, NFR-151, NFR-165, NFR-166, NFR-180, NFR-181, NFR-191, NFR-192, NFR-206, NFR-207 |
| Security | NFR-6, NFR-20, NFR-21, NFR-38, NFR-43, NFR-44, NFR-59, NFR-72, NFR-73, NFR-88, NFR-107, NFR-117, NFR-118, NFR-133, NFR-135, NFR-147, NFR-148, NFR-153, NFR-167, NFR-168, NFR-183, NFR-194, NFR-195, NFR-209 |
| Data / residency | NFR-8, NFR-22, NFR-40, NFR-46, NFR-61, NFR-76, NFR-91, NFR-120, NFR-137, NFR-159, NFR-155, NFR-172, NFR-186, NFR-196, NFR-211 |
| Deployment / environment | NFR-9, NFR-23, NFR-44, NFR-47, NFR-62, NFR-77, NFR-92, NFR-102, NFR-122, NFR-138, NFR-160, NFR-156, NFR-173, NFR-187, NFR-197, NFR-213 |
| Auditability / data integrity | NFR-5, NFR-7, NFR-25, NFR-39, NFR-45, NFR-60, NFR-74, NFR-90, NFR-121, NFR-139, NFR-154, NFR-158, NFR-171, NFR-184, NFR-212, NFR-214 |
| Recovery / retention | NFR-11, NFR-14, NFR-42, NFR-45, NFR-64, NFR-79, NFR-94, NFR-157, NFR-193, NFR-200, NFR-215 |
...
- **US-10:** As an operator, I want a guided, multi-step project creation wizard, so that I can create a new pipeline project without missing required information.
...
- **US-11:** As an operator, I want to capture the seed idea and optional structured goal fields, so that the project starts with clear intent.
...
- **US-12:** As an operator, I want to enter and validate project metadata, so that the project is identifiable and governed correctly.
...
- **US-13:** As an operator, I want to assign the new project to a portfolio during creation, so that it is grouped immediately.
...
- **US-14:** As an operator, I want to bind a pipeline definition and pinned version at creation, so that the project uses the correct pipeline.
...
- **US-15:** As an operator, I want to select a predefined model tier, so that I can control cost and capability trade-offs.
...
- **US-16:** As an operator, I want to customize a model tier, so that I can tune model and provider choices for a project.
...
- **US-17:** As an operator, I want to create a new tier from the list of LLM models and providers, so that I can define reusable tiers for future projects.
...
- **US-25:** As an operator, I want to initiate a run from an existing project, so that the pipeline starts executing.
...
- **US-26:** As an operator, I want pre-flight readiness validation before a run starts, so that I can fix missing prerequisites first.
...
- **US-27:** As an operator, I want the run plan to be materialized from the pinned pipeline version, so that execution is deterministic.
...
- **US-28:** As an operator, I want live progress and telemetry streaming, so that I can monitor the run as it happens.
...
- **US-29:** As an operator, I want human-in-the-loop gates to pause the run for decisions, so that humans can intervene at critical points.
...
- **US-30:** As an operator, I want a complete, auditable record of everything that happened during the run, so that I can investigate and prove outcomes.
...
- **US-31:** As a portfolio manager, I want to create and govern portfolios, so that I can organize projects into durable groups.
...
- **US-32:** As a portfolio manager, I want to add and remove member projects, so that the portfolio always reflects the intended set.
...
- **US-33:** As a portfolio manager, I want aggregated roll-ups across member projects, so that I can see portfolio health at a glance.
...
- **US-34:** As a portfolio manager, I want to issue scoped bulk operations over member projects and runs, so that I can act at portfolio scale.
...
- **US-35:** As a portfolio manager, I want authorization scoped to each portfolio, so that only permitted users can view or manage it.
...
- **US-36:** As a portfolio manager, I want reporting on portfolio state, so that I can share status with stakeholders.
...
- **US-46:** As an operator, I want to create a multi-project run group, so that I can coordinate the same or compatible pipeline across many projects.
...
- **US-47:** As an operator, I want to define project scope and execution mode, so that the group targets exactly the intended projects.
...
- **US-48:** As an operator, I want child runs to be created per project using F-3 rules, so that each project executes correctly.
...
- **US-49:** As an operator, I want shared and overridden parameters, so that I can customize behavior per project where needed.
...
- **US-50:** As an operator, I want failure policy enforced across the group, so that group behavior is predictable when some children fail.
...
- **US-51:** As an operator, I want aggregated monitoring and HIL gates for the group, so that I can manage all children centrally.
...
- **US-52:** As an auditor, I want a complete parent-child audit trail, so that I can trace group execution end to end.
...
- **US-61:** As an operator, I want to supply manual input to a live run or stage, so that I can unblock work that requires human-provided data.
...
- **US-62:** As an operator, I want to decide HIL gates, so that runs can proceed past human-approval points.
...
- **US-63:** As an operator, I want to pause, resume, and cancel runs or stages, so that I can control execution interactively.
...
- **US-64:** As an operator, I want to retry or skip stages, so that I can recover from failures or bypass non-critical work.
...
- **US-65:** As an operator, I want to override parameters on a run or stage, so that I can correct configuration without restarting everything.
...
- **US-66:** As an operator, I want an emergency-wide halt, so that I can immediately stop unsafe or unwanted activity across the system.
...
- **US-67:** As an operator, I want dry-run/preview and confirmation for manual actions, so that I can avoid destructive mistakes.
...
- **US-68:** As an operator, I want idempotent, auditable commands with conflict handling, so that retries and concurrent actions are safe.
...
- **US-69:** As a security administrator, I want manual actions gated by role, so that only authorized users can intervene.
...
- **US-76:** As an operator, I want to choose and bind an autonomy level, so that Auto Mode operates within my risk tolerance.
...
- **US-78:** As an operator, I want Auto Mode to decide HIL gates according to policy, so that runs can progress without manual gate handling.
...
- **US-79:** As an operator, I want Auto Mode to auto-retry, auto-skip, auto-pause, auto-halt, and auto-escalate according to policy, so that failures are handled consistently.
...
- **US-80:** As an operator, I want budget, time, and risk guardrails, so that Auto Mode cannot exceed safe operating limits.
...
- **US-81:** As an operator, I want bounded automatic parameter overrides, so that minor issues can self-heal without unbounded changes.
...
- **US-82:** As an auditor, I want an immutable, explainable decision ledger for Auto Mode, so that I can review why every autonomous decision was made.
...
- **US-83:** As an operator, I want a kill switch for immediate manual takeover, so that I can stop Auto Mode instantly.
...
- **US-84:** As an operator, I want Auto Mode to fail closed toward escalation, so that ambiguous situations do not cause unsafe autonomous actions.
...
- **US-91:** As a dashboard user, I want a versioned design-token foundation, so that the UI is visually consistent across all surfaces.
...
- **US-92:** As a dashboard user, I want a primitive, composed, and feature component library, so that pipeline state is rendered consistently.
...
- **US-93:** As a dashboard user, I want clear loading, empty, and error states, so that I always understand system status.
...
- **US-94:** As a dashboard user, I want real-time updates, so that I see live pipeline changes without manual refresh.
...
- **US-95:** As a dashboard user, I want theming and responsive layouts, so that the UI works on my device and preferences.
...
- **US-96:** As an accessibility user, I want keyboard navigation, ARIA labels, sufficient contrast, and visible focus indicators, so that I can use the dashboard independently.
...
- **US-97:** As an international user, I want locale-aware dates, times, numbers, and RTL readiness, so that the UI fits my language and region.
...
- **US-98:** As a motion-sensitive user, I want reduced-motion support, so that animations do not cause discomfort.
...
- **US-106:** As an operator, I want to open the AI companion from any dashboard surface, so that I can get help without leaving my current task.
...
- **US-107:** As an operator, I want the companion automatically bound to my current context, so that its answers are relevant.
...
- **US-108:** As an operator, I want to ask questions about authoritative pipeline state, so that I can understand what is happening.
...
- **US-109:** As an operator, I want the companion to explain behavior and diagnose failures, so that I can resolve issues faster.
...
- **US-110:** As an operator, I want the companion to draft commands for the manual command surface, so that I can act without it executing commands directly.
...
- **US-111:** As an operator, I want grounded answers with citations, so that I can trust the information.
...
- **US-112:** As an operator, I want the companion to refuse inventing state, commands, or exceeding my scope, so that it remains safe.
...
- **US-113:** As an operator, I want graceful degradation to deterministic help and search when AI is unavailable, so that I can still get assistance.
...
- **US-121:** As an operator, I want hands-free voice activation, so that I can talk to the companion without using the keyboard.
...
- **US-122:** As an operator, I want streaming speech-to-text, so that my speech is transcribed accurately and promptly.
...
- **US-123:** As an operator, I want spoken synthesis of grounded answers with citations, so that I can hear responses.
...
- **US-124:** As an operator, I want barge-in, so that I can interrupt speech and continue the conversation naturally.
...
- **US-125:** As an operator, I want hands-free HIL gate announcements and decisions, so that I can handle gates by voice.
...
- **US-126:** As an operator, I want voice-driven navigation, so that I can move around the dashboard hands-free.
...
- **US-127:** As an operator, I want voice drafting of commands for the manual command surface, so that I can prepare actions by voice.
...
- **US-128:** As a multilingual operator, I want multilingual speech support, so that I can use the companion in my language.
...
- **US-129:** As a privacy-conscious operator, I want transcript and audio privacy controls, so that my voice data is protected.
...
- **US-130:** As an operator, I want graceful degradation to text, so that voice failures do not block me.
...
- **US-133:** As a mobile operator, I want a secure, responsive view of pipeline state, so that I can monitor projects, runs, gates, and portfolios on the go.
...
- **US-134:** As a mobile operator, I want notifications for gates, failures, Auto Mode decisions, and run completion, so that I can react promptly.
...
- **US-135:** As a mobile operator, I want read-first, action-safe behavior, so that I do not accidentally execute commands.
...
- **US-136:** As a mobile operator, I want commands delegated to the manual command surface, so that every action is previewed, confirmed, and audited.
...
- **US-137:** As a mobile operator, I want offline read-only access to last-known state, so that I can review without connectivity.
...
- **US-138:** As a mobile operator, I want graceful degradation when connectivity or AI services are unavailable, so that the app remains useful.
...
- **US-139:** As a test manager, I want to register and version test cases, so that tests are reusable and traceable.
...
- **US-140:** As a test manager, I want to compose test cases into suites and plans, so that I can organize and execute testing coherently.
...
- **US-141:** As a test manager, I want to bind test plans to pipeline stages and projects, so that tests run in the right context.
...
- **US-142:** As a test manager, I want tests to run on demand, on schedule, or as a governed pipeline stage, so that testing fits the workflow.
...
- **US-143:** As a test manager, I want deterministic outcomes and evidence capture, so that results are reliable and reviewable.
...
- **US-144:** As a quality gate owner, I want plan-level quality gates that stage advancement consumes, so that pipelines do not advance on failed quality.
...
- **US-145:** As a test manager, I want coverage tracking over pipeline artifacts, so that I can see what is tested and what is not.
...
- **US-146:** As a test manager, I want regression and flakiness management, so that unstable tests are identified and controlled.
...
- **US-147:** As a test manager, I want waiver and approval workflows, so that exceptions to quality gates are governed.
...
- **US-148:** As an auditor, I want an immutable test audit trail, so that I can review every test definition, execution, and gate decision.
...
- **US-154:** As an API consumer, I want a versioned, documented API, so that I can integrate reliably over time.
...
- **US-155:** As an API consumer, I want contract publication, so that I can generate clients and validate compatibility.
...
- **US-156:** As an API consumer, I want authentication and scope-based authorization, so that access is controlled and least-privilege.
...
- **US-157:** As an API consumer, I want credential lifecycle management, so that I can rotate and revoke secrets safely.
...
- **US-158:** As an API consumer, I want a uniform request/response envelope with pagination, filtering, and sparse fieldsets, so that responses are predictable and efficient.
...
- **US-159:** As an API consumer, I want idempotency and concurrency control, so that retries and concurrent updates are safe.
...
- **US-160:** As an API consumer, I want asynchronous long-running-operation patterns, so that slow work can be managed without blocking.
...
- **US-161:** As an API consumer, I want a streaming surface for live run telemetry, so that I can monitor runs in real time.
...
- **US-162:** As an API consumer, I want webhooks and event subscriptions, so that I can react to changes without polling.
...
- **US-163:** As an API consumer, I want bulk operations and dry-run preview, so that I can act at scale safely.
...
- **US-164:** As an API consumer, I want rate limiting, quotas, and capability discovery, so that usage is predictable and discoverable.
...
- **US-165:** As an API consumer, I want SDK/CLI generation and a sandbox environment, so that I can develop and test quickly.
...
- **US-166:** As an auditor, I want API activity auditing, so that I can trace API access and changes.
...
- **US-169:** As an API governance analyst, I want an inventory snapshot of published API contracts, so that I know what capabilities already exist.
...
- **US-170:** As an API governance analyst, I want a demand registry, so that I can track requested or expected capabilities from consumers.
...
- **US-171:** As an API governance analyst, I want matching and gap detection between demand and contracts, so that I can find missing capabilities.
...
- **US-172:** As an API governance analyst, I want reuse/extension classification, so that I can decide whether to reuse, extend additively, version, or introduce new API.
  - **Acceptance Criteria:** Each demand is classified with rationale; classification respects versioning and compatibility rules.
- **US-173:** As an API governance analyst, I want duplication and overlap findings, so that I can reduce redundant API surface.
  - **Acceptance Criteria:** Findings identify overlapping operations/schemas; severity and recommended consolidation are provided.
- **US-174:** As an API governance analyst, I want per-area coverage scores, so that I can prioritize governance work.
...
- **US-175:** As an API governance analyst, I want review and decision records, so that reuse decisions are traceable.
  - **Acceptance Criteria:** Decision records include demand, analysis, outcome, approver, timestamp, and linked contract/version.
- **US-176:** As an API governance analyst, I want published reuse guidance, so that teams can follow consistent API evolution rules.
...
- **US-180:** As a platform operator, I want a deployment-target catalog, so that I can choose where Product Forge runs.
...
- **US-181:** As a platform operator, I want topology and sizing profiles, so that I can match deployment capacity to expected load.
...
- **US-182:** As a platform operator, I want provisioning and bootstrap paths per target, so that I can install reliably.
...
- **US-183:** As a platform operator, I want configuration and secret management, so that deployments are secure and maintainable.
...
- **US-184:** As a platform operator, I want upgrade and rollback, so that I can maintain versions safely.
...
- **US-185:** As a platform operator, I want scaling, high availability, and backup/restore, so that the platform is resilient.
...
- **US-186:** As a platform operator, I want observability, network/TLS, and identity wiring, so that operations are visible and secure.
...
- **US-187:** As a compliance officer, I want data-residency placement and air-gapped/offline delivery, so that regulatory and isolation requirements are met.
...
- **US-188:** As a platform operator, I want entitlement binding, drift detection, and multi-environment promotion, so that deployments stay compliant and consistent.
...
- **US-189:** As a platform operator, I want hardening profiles and cost estimation, so that deployments are safe and affordable.
...
- **US-190:** As a platform operator, I want decommissioning, so that I can retire deployments cleanly.
...
- **US-195:** As a voice engineer, I want to register a target voice and locale, so that I can create a reference profile.
...
- **US-196:** As a voice engineer, I want to ingest source audio samples, so that I can build the reference from real speech.
...
- **US-197:** As a voice engineer, I want preprocessing, normalization, phonetic/transcript alignment, and speaker embeddings, so that the reference is technically accurate.
...
- **US-198:** As a voice engineer, I want objective alignment scoring and pass/review/fail verdicts, so that quality is measured consistently.
...
- **US-199:** As a voice engineer, I want immutable reference-profile versions, so that changes are traceable and reproducible.
...
- **US-200:** As a voice engineer, I want approval and retirement workflows, so that reference profiles are governed.
...
- **US-201:** As a voice engineer, I want to bind profiles to languages/locales and Voice Companion resolution rules, so that the correct voice is used.
...
- **US-202:** As a voice engineer, I want drift monitoring and cross-sample consistency checks, so that reference quality is maintained over time.
...
- **US-203:** As a voice engineer, I want search, import, and export of voice references, so that I can manage references efficiently.
...
- **US-204:** As a security administrator, I want role-gated access and an immutable audit trail for voice references, so that sensitive voice data is controlled.
...
- **US-205:** As a voice engineer, I want settings/presets, batch alignment, failure diagnosis, preview synthesis, and completion notifications, so that I can operate efficiently at scale.
